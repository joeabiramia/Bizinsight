...
def create_goal(
    ...
    insert_goal(goal)
    log_action(current_user["user_id"], "goal_created", "goal", goal["goal_id"], {"name": body.name})
...
def update_goal_endpoint(
    ...
    updated = update_goal(goal_id, current_user["user_id"], updates)
    if not updated:
        raise HTTPException(status_code=404, detail="Goal not found")
    log_action(current_user["user_id"], "goal_updated", "goal", goal_id)
...
def delete_goal_endpoint(
    ...
    deleted = delete_goal(goal_id, current_user["user_id"])
    if not deleted:
        raise HTTPException(status_code=404, detail="Goal not found")
    log_action(current_user["user_id"], "goal_deleted", "goal", goal_id)
